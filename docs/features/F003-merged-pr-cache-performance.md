...
1. **operator 最初要解决的核心问题是什么？** 大范围重复查询等待过久；已合入
   PR 不应因为极少见的 merge 后评论变化而反复拉取；8/16 并发不能在内部
   fan-out 中失控；结果出现后进度必须结束；用户映射变化不应触发重查。
2. **交付物解决了吗？** 是。PR #1 已合入，222/222 测试通过；GitHub Pages
   首查为缓存 0 / 网络 1，复查为缓存 1 / 网络 0，详情与三类协作请求未增加；
   用户编辑即时重映射且请求计数不变。
3. **operator 使用体验如何？** 首次查询沿用原入口并提高实际并行度；重复查询
   自动复用缓存，无需新增刷新操作；进度明确进入终态；用户管理后的当前表格
   即时更新。生产 journey 控制台无错误。

## User Visibility Disclosure

| Surface | 用户能做什么（达成态） | F003 close 时实际行为 | 缺失/退化 | 处置 |
|---------|-------------------------|----------------------|-----------|------|
| 查询进度 | 看到真实完成数、缓存/网络计数与唯一终态 | GitHub Pages 首查与复查均显示完成，分项计数等于 PR 总数 | 无 | met |
| 重复查询 | 自动复用完整的已合入 PR，跳过 PR 详情与协作请求 | HTTP(S) 跨重开持久；同页复查为缓存 1 / 网络 0 | `file://` 关闭页面后缓存失效 | 明确的运行边界与 non-goal；README/界面已披露 |
| 用户管理 | 编辑、启禁用、删除或导入后立即更新当前结果 | 当前结果本地重映射，Provider/fetch 请求为 0 | 无 | met |
| 缓存隐私 | 知道保存内容、位置和清理方式 | 配置页与 README 说明 IndexedDB、raw activity、浏览器站点数据清理 | 无 | met |

没有 deliberate defer 或用户可见的未交付 surface。
...
| 2026-07-26 | 实现完成：222/222 浏览器测试通过；本地 Chrome 首查、缓存复查与用户重映射 journey 通过 |
| 2026-07-26 | fresh-context 扫描 10 条 P3 全部处置；修正 GitHub 网络计数、缓存结构门禁与重复筛选刷新，并补仓库并行及嵌套 fan-out 测试 |
| 2026-07-26 | PR #1 squash merged as `b3b877a`；kimi 与 Opus 分别对 `a576600` 独立 APPROVE |
| 2026-07-26 | GitHub Pages 发布 `0c46185` 成功；生产 `index.html` / `test.html` 与本地 SHA-256 一致，222/222 测试与首查→缓存复查→用户零网络重映射 journey 通过 |
...
| **Quality Gate** | `docs/quality-gates/2026-07-26-F003.md` | 测试、review 与生产验收证据 |
| **Reflection** | `project-reflections/2026-07-26-f003-merged-pr-cache-performance-capsule.md` | 完成反思胶囊 |
